...
public:
	frame_equalizer_impl(Equalizer algo, double freq, double bw, bool log, bool debug, unsigned int nin);
...
private:
	...
	std::vector<equalizer::base *> d_equalizer_pass;  //each d_equlizer_pass[i] point to a base class, for every channel use.

	gr::thread::mutex d_mutex;
	std::vector<gr::tag_t> *tags;
	gr_complex **d_long_128;
	...
	double *d_freq_offset_from_synclong_pass;
	double *d_er_pass;
	double *d_epsilon0_pass;
	gr_complex (*d_prev_pilots_pass)[4];
	int d_cnt;
